...
frappe.provide("erplite.setup");
erplite.setup.EmployeeController = class EmployeeController extends frappe.ui.form.Controller {
	setup() {
		...
		this.frm.fields_dict.reports_to.get_query = function (doc, cdt, cdn) {
			return { query: "erplite.controllers.queries.employee_query" };
		};
	}
...
frappe.ui.form.on("Employee", {
	...
	create_user: function (frm) {
		...
		frappe.call({
			method: "erplite.setup.doctype.employee.employee.create_user",
			args: {
...
cur_frm.cscript = new erplite.setup.EmployeeController({
	frm: cur_frm,
});
