...
frappe.provide("erplite.buying");
// cur_frm.add_fetch('project', 'cost_center', 'cost_center');

erplite.buying = {
	setup_buying_controller: function() {
		erplite.buying.BuyingController = class BuyingController extends erplite.TransactionController {
			setup() {
				super.setup();
				...
				if(this.frm.get_field('shipping_address')) {
					this.frm.set_query("shipping_address", () => {
						if(this.frm.doc.customer) {
							...
							return erplite.queries.company_address_query(this.frm.doc)
					});
				}
			}
			...
			setup_queries(doc, cdt, cdn) {
				...
				me.frm.set_query('supplier', erplite.queries.supplier);
				me.frm.set_query('contact_person', erplite.queries.contact_query);
				me.frm.set_query('supplier_address', erplite.queries.address_query);

				me.frm.set_query('billing_address', erplite.queries.company_address_query);
				erplite.accounts.dimensions.setup_dimension_filters(me.frm, me.frm.doctype);
				...
				this.frm.set_query("manufacturer", "items", function(doc, cdt, cdn) {
					...
					return {
						query: "erplite.controllers.queries.item_manufacturer_query",
			...
			supplier() {
				var me = this;
				erplite.utils.get_party_details(this.frm, null, null, function(){
					me.apply_price_list();
				});
			}

			supplier_address() {
				erplite.utils.get_address_display(this.frm);
				erplite.utils.set_taxes_from_address(this.frm, "supplier_address", "supplier_address", "supplier_address");
			}
			...
			warehouse(doc, cdt, cdn) {
				...
				if(item.item_code && item.warehouse) {
					return this.frm.call({
						method: "erplite.stock.get_item_details.get_bin_details",
						child: item,
						args: {
			...
			set_from_product_bundle() {
				...
				this.frm.add_custom_button(__("Product Bundle"), function() {
					erplite.buying.get_items_from_product_bundle(me.frm);
				}, __("Get Items From"));
			}

			shipping_address(){
				var me = this;
				erplite.utils.get_address_display(this.frm, "shipping_address",
					"shipping_address_display", true);
			}

			billing_address() {
				erplite.utils.get_address_display(this.frm, "billing_address",
			...
			manufacturer(doc, cdt, cdn) {
				...
				if(row.manufacturer) {
					frappe.call({
						method: "erplite.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
						args: {
				...
				frappe.db.get_value("Item", item.item_code, fields)
					.then((r) => {
						if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
							...
							new erplite.SerialBatchPackageSelector(
								me.frm, item, (r) => {
									if (r) {
...
erplite.buying.link_to_mrs = function(frm) {
	frappe.call({
		method: "erplite.buying.utils.get_linked_material_requests",
		args:{
...
erplite.buying.get_default_bom = function(frm) {
	$.each(frm.doc["items"] || [], function(i, d) {
		if (d.item_code && d.bom === "") {
			return frappe.call({
				type: "GET",
				method: "erplite.stock.get_item_details.get_default_bom",
				args: {
...
erplite.buying.get_items_from_product_bundle = function(frm) {
	var dialog = new frappe.ui.Dialog({
		...
		primary_action(args){
			...
			return frappe.call({
				type: "GET",
				method: "erplite.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
