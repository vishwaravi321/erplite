...
from erplite.accounts.doctype.loyalty_program.loyalty_program import validate_loyalty_points
...
from erplite.controllers.queries import item_query as _item_query
...
class POSInvoice(SalesInvoice):
	...
	if TYPE_CHECKING:
		...
		from erplite.accounts.doctype.pos_invoice_item.pos_invoice_item import POSInvoiceItem
		from erplite.accounts.doctype.pricing_rule_detail.pricing_rule_detail import PricingRuleDetail
		from erplite.accounts.doctype.sales_invoice_advance.sales_invoice_advance import (
			SalesInvoiceAdvance,
		)
		from erplite.accounts.doctype.sales_invoice_payment.sales_invoice_payment import (
			SalesInvoicePayment,
		)
	...
	def validate(self):
		...
		if self.coupon_code:
			from erplite.accounts.doctype.pricing_rule.utils import validate_coupon_code
	...
	def on_submit(self):
		...
		if self.coupon_code:
			from erplite.accounts.doctype.pricing_rule.utils import update_coupon_code_count
