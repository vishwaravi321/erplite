...
from erplite.accounts.party import get_due_date, get_party_account, get_party_details
from erplite.accounts.utils import cancel_exchange_gain_loss_journal, get_account_currency
...
class SalesInvoice(SellingController):
	...
	def set_pos_fields(self, for_validate=False):
		...
		from erplite.stock.get_item_details import get_pos_profile, get_pos_profile_item_details
	...
	def make_gl_entries(self, gl_entries=None, from_repost=False):
		from erplite.accounts.general_ledger import make_gl_entries, make_reverse_gl_entries
	...
	def get_gl_entries(self, warehouse_account=None):
		from erplite.accounts.general_ledger import merge_similar_entries
	...
	def make_item_gl_entries(self, gl_entries):
		...
		if cint(self.update_stock) and erplite.is_perpetual_inventory_enabled(self.company):
	...
	def apply_loyalty_points(self):
		from erplite.accounts.doctype.loyalty_point_entry.loyalty_point_entry import (
...
def get_loyalty_programs(customer):
	...
	from erplite.selling.doctype.customer.customer import get_loyalty_programs
...
def create_dunning(source_name, target_doc=None, ignore_permissions=False):
	...
	def postprocess_dunning(source, target):
		from erplite.accounts.doctype.dunning.dunning import get_dunning_letter_text
