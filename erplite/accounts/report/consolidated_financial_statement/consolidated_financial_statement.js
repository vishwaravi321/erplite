...
frappe.query_reports["Consolidated Financial Statement"] = {
	...
	formatter: function (value, row, column, data, default_formatter) {
		if (data && column.fieldname == "account") {
			...
			column.link_onclick =
				"erplite.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			column.is_tree = true;
		}

		if (data && data.account && column.apply_currency_formatter) {
			data.currency = erplite.get_currency(column.company_name);
		}
	...
	onload: function () {
		let fiscal_year = erplite.utils.get_fiscal_year(frappe.datetime.get_today());
