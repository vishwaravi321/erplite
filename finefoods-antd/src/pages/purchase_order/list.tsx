...
import {
  ...
  import { POrderStatus, PaginationTotal } from "../../components";
  ...
  export const PurchaseOrderList = () => {
    ...
    const { data,isLoading,mutate } = useFrappeGetDocList<any>('Purchase Order', {
        fields: ['name', 'status', 'grand_total', 'supplier' , 'creation', 'modified_by'],
        limit:1000,
        asDict:true
      });

    useFrappeDocTypeEventListener('Purchase Order', (d) => {
        console.log("Event", d)
        if (d.doctype === "Purchase Order") {
            mutate()
        }
      })
            ...
            <>
                  <Table
                    ...
                    onRow={(record) => {
                      return {
                        onClick: () => {
                          show("purchase_order", record.name);
                        },
                      };
                    }}
                    pagination={{
                      showTotal: (total) => (
                        <PaginationTotal total={total} entityName="Purchase Order" />
                      ),
                    }}
                  >
                    ...
                    <Table.Column
                      ...
                      title={("Status")}
                      render={(status) => {
                        return <POrderStatus status={status} />;
                      }}
                    />
