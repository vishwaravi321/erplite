...
import {
  ...
  export const ItemGroupList = () => {
    ...
    const { data,isLoading,mutate } = useFrappeGetDocList<any>('Item Group', {
        fields: ['name', 'item_group_name', 'parent_item_group', 'is_group' , 'creation', 'modified_by'],
        limit:1000,
        asDict:true
      });
    ...
    const { triggerExport } = useExport<IItemGroup>({
        pageSize: 50,
        maxItemCount: 50,
        mapData: (item) => {
          return {
            ...
            <>
                  <Table
                    ...
                    onRow={(record) => {
                      return {
                        onClick: () => {
                          show("item_group", record.name);
                        },
                      };
                    }}
                    pagination={{
                      showTotal: (total) => (
                        <PaginationTotal total={total} entityName="ItemGroup" />
                      ),
                    }}
                  >
