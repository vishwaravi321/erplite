...
import {
  ...
  export const SupplierList = () => {
    ...
    const { data,isLoading,mutate } = useFrappeGetDocList<any>('Supplier', {
        fields: ['name', 'disabled', 'supplier_group', 'supplier_name','creation', 'modified_by'],
        limit:1000,
        asDict:true
      });

    useFrappeDocTypeEventListener('Supplier', (d) => {
        console.log("Event", d)
        if (d.doctype === "Supplier") {
            mutate()
        }
      })
            ...
            <>
                  <Table
                    ...
                    onRow={(record) => {
                      return {
                        onClick: () => {
                          show("supplier", record.name);
                        },
                      };
                    }}
                    pagination={{
                      showTotal: (total) => (
                        <PaginationTotal total={total} entityName="Supplier" />
                      ),
                    }}
                  >
