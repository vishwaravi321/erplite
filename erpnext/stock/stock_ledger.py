...
import erplite
from erplite.stock.doctype.bin.bin import update_qty as update_bin_qty
...
from erplite.stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero
...
class update_entries_after:
	...
	def update_batched_values(self, sle):
		from erplite.stock.serial_batch_bundle import BatchNoValuation
	...
	def get_fallback_rate(self, sle) -> float:
		...
		return get_valuation_rate(
			...
			currency=erplite.get_company_currency(sle.company),
			company=sle.company,
		)
...
def get_valuation_rate(
	...
	if (
		...
		and cint(erplite.is_perpetual_inventory_enabled(company))
	):
