...
import PurchaseOrderModal from './PurchaseOrderModel';
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const frappe = new FrappeApp("http://162.55.41.54");
        const db = frappe.db();
        const data = await db.getDocList('Purchase Order', {
          fields: ['name', 'status', 'grand_total','transaction_date', 'supplier_name', 'creation', 'project'],
          orderBy: {
            field: 'creation',
            order: 'desc',
          },
